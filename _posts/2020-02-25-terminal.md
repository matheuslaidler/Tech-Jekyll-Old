---
layout: post
title: Dominando terminal linux
categories:
- linux
...
Já o **|** *(**pipe**)* acaba sendo usada mais como um separador para filtros para um comando. O pipe pode tornar os comandos mais práticos. A sua função nada mais é que fazer uma comunicação entre processos. A exibição de um comando vai variar dependendo do comando dado após o pipe, como um filtro. Se eu estiver nos meus documentos e botar um ls, mostrará tudo que tem lá. Se eu só quiser a listagem de arquivos que tenham "aula" no nome, colocando ""ls | grep aula" irá listar apenas os arquivos e documentos que tenham aula no nome.

Outro exemplo (da net): **$ ls | grep b | sort -r | tee arquivo.out | wc -l**
